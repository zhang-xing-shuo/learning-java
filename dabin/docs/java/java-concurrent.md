...
Java并发,多线程,线程池执行原理,线程池参数,线程和进程,死锁,volatile,synchronized,ThreadLocal,Java锁,Java并发工具,原子类,AQS
...
1. 不受控
   ...
   系统资源是有限的，每个人针对不同业务都可以手动创建线程。创建线程没有统一标准，比如创建的线程有没有名字等。当系统运行起来，所有线程都在抢占资源，毫无规则，混乱场面可想而知，不好管控。

2. 频繁创建开销大

   Java中，万物皆对象。new Thread()创建一个线程和 new Object() 还是有区别的
...
- **降低资源消耗**
  不断创建新的线程并销毁，消耗许多资源。重复利用已创建的线程，将降低消耗的资源。
- **提高响应速度**
  当需要一个线程时，直接返回已创建好的线程。中间，省去了创建新的线程的时间。
- **提高线程的可管理性**
